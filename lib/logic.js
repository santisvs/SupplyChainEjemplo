...
async function venderJamon(tx){
    ...
    if (mismaDivisa(tx.jamon,tx.propietarioRestaurante,tx.ganadero)) {
        if (tieneDinero(tx.jamon,tx.propietarioRestaurante)){
      ...
      try {
        ...
        tx.ganadero.dinero.cantidad = tx.ganadero.dinero.cantidad + tx.jamon.precio.cantidad
        tx.propietarioRestaurante.dinero.cantidad = tx.propietarioRestaurante.dinero.cantidad - tx.jamon.precio.cantidad
        ...
        await restauranteOwnerRegistry.update(restauranteOwnerRegistryAux);
        	...
        	throw new Error(`tiene dinero jamon ${tx.jamon.precio.cantidad} prop ${tx.propietarioRestaurante.contabilidad.cantidad} does not exist`);
        }
    } else {
    	throw new Error(`misma divisa id ${tx.jamon.getIdentifier()} does not exist`);
    }
